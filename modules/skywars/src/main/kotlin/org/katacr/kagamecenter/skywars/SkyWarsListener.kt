...
/** 将 Bukkit 事件按玩家或临时世界精确路由到所属 SkyWars Session。 */
class SkyWarsListener(private val roomManager: GameRoomManager) : Listener {
    ...
    private fun session(player: Player): SkyWarsGameSession? {
        val room = roomManager.getPlayerRoom(player) ?: return null
        return room.session as? SkyWarsGameSession
    }

    private fun session(world: World): SkyWarsGameSession? {
